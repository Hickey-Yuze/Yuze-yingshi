...
export async function generateDoubanSignature(urlPath, timestamp, method = "GET") {
  ...
  return await hmacSha1Base64(secretKey, rawSign);
}
...
export async function getDoubanActors(doubanId) {
  ...
  const signature = await generateDoubanSignature(url_path, timestamp);
  // 构造请求参数
  const params = new URLSearchParams({
    apiKey: api_key,
    os_rom: "android",
