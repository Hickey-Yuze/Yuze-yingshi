...
export function createDanmakuLoader(
  ...
  if (sourceUrl.includes('danmu.icu') && currentVideoUrl) {
    ...
    finalDanmuUrl = `${baseUrl}/?ac=dm&url=${encodeURIComponent(currentVideoUrl)}`;
...
function createDanmakuLoaderForGengYu(sourceUrl, videoTitle, episodeTitle, episodeNumber) {
  ...
  const searchUrl = `${baseUrl}/api/v2/search/anime?keyword=${encodeURIComponent(searchKeyword)}`;
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (!searchData.animes || searchData.animes.length === 0) {
          ...
          const fallbackUrl = `${baseUrl}/api/v2/search/anime?keyword=${encodeURIComponent(videoTitle)}`;
        ...
        const detailUrl = `${baseUrl}/api/v2/bangumi/${animeId}`;
        ...
        const danmakuUrl = `${baseUrl}/api/v2/comment/${episodeId}`;
