...
async function createDanmakuLoaderForGengYu(sourceUrl, doubanId, episodeTitle, episodeNumber) {
  const searchUrl = `${sourceUrl}/api/v2/search/anime?keyword=${encodeURIComponent(episodeTitle.replace(/\(.*?\)/g, '').trim())}`;
  console.log("搜索动漫URL:", searchUrl);
