...
function convertDanmakuFormat(rawDanmaku) {
  if (!Array.isArray(rawDanmaku)) {
    console.log("弹幕数据不是数组:", rawDanmaku);
    return [];
  }
  console.log("开始转换弹幕，共", rawDanmaku.length, "条");
  console.log("第一条弹幕:", rawDanmaku[0]);

  return rawDanmaku
    .map((item, index) => {
      if (!Array.isArray(item)) {
        console.log(`第${index}条不是数组:`, item);
        return null;
      }
      if (item.length < 5) {
        console.log(`第${index}条长度不足:`, item.length, item);
        return null;
      }
      ...
      if (index < 3) {
        console.log(`第${index}条: time=${time}, position=${position}, color=${color}, fontSize=${fontSize}, text=${text}`);
      }
...
export function createDanmakuLoader(
  ...
  if (!videoTitle && (!doubanId || !episodeTitle || enabledSources.length === 0)) {
  ...
  console.log("弹幕源 URL:", sourceUrl, "视频标题:", videoTitle, "集数标题:", episodeTitle);
  ...
  if (sourceUrl.includes('danmu.icu') && currentVideoUrl) {
    ...
    return createDanmakuLoaderForGengYu(sourceUrl, videoTitle, episodeTitle, episodeNumber);
  } else {
...
function createDanmakuLoaderForGengYu(sourceUrl, videoTitle, episodeTitle, episodeNumber) {
  const searchKeyword = videoTitle.replace(/\(.*?\)/g, '').trim();
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (!searchData.animes || searchData.animes.length === 0) {
          ...
          const fallbackUrl = `${sourceUrl}/api/v2/search/anime?keyword=${encodeURIComponent(videoTitle)}`;
        ...
        const rawDanmaku = danmakuData.comments || danmakuData.danmuku || [];
        const convertedData = convertDanmakuFormat(rawDanmaku);
        console.log(`成功获取 ${convertedData.length} 条弹幕`);
        resolve(convertedData);
      } catch (error) {
        console.error("获取弹幕失败:", error);
